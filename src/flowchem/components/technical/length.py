...
class LengthControl(FlowchemComponent):
    ...
    def __init__(
        ...
        mode: str = "discrete",
        _available_positions: list = [int | float | str],
    ) -> None:
        """
        ...
        if self.mode not in ["discrete", "continuous"]:
            logger.error(
                f"Invalid mode: {self.mode}. Must be 'discrete' or 'continuous'."
            )
        if self.mode == "discrete":
            if not all(
                isinstance(pos, (int, float, str)) for pos in _available_positions
            ):
                logger.error(
                    "All positions must be numbers or strings in discrete mode."
                )
        elif self.mode == "continuous":
            if (
                len(_available_positions) != 2
                or _available_positions[0] >= _available_positions[1]
            ):
                logger.error(
        ...
        self.add_api_route(
            "/get_available_positions", self.get_available_positions, methods=["GET"]
        )
        ...
        def validate_position(
            ...
            if mode == "continuous":
                if not isinstance(position, float):
                    logger.error(
                        f"Invalid type for {axis_name}-axis in continuous mode: {position}. Must be a float."
                    )
